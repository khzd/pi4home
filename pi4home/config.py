...
from pi4home.core import ConfigType  # noqa
from pi4home.yaml_util import is_secret
from pi4home.voluptuous_schema import ExtraKeysInvalid
...
def get_component(domain):
    ...
    path = 'pi4home.components.{}'.format(domain)
    try:
...
def iter_components(config):
    for domain, conf in config.items():
        if domain == CONF_PI4HOME:
            yield CONF_PI4HOME, core_config, conf
            continue
...
def do_id_pass(result):  # type: (Config) -> None
    from pi4home.cpp_generator import MockObjClass
...
def validate_config(config):
    ...
    result.add_domain([CONF_PI4HOME], CONF_PI4HOME)
    result[CONF_PI4HOME] = config[CONF_PI4HOME]

    for domain, conf in config.items():
        domain = str(domain)
        if domain == CONF_PI4HOME or domain.startswith(u'.'):
    ...
    try:
        result[CONF_PI4HOME] = core_config.CONFIG_SCHEMA(result[CONF_PI4HOME])
    except vol.Invalid as ex:
        _comp_error(ex, [CONF_PI4HOME])
