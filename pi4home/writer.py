...
from pi4home.config import iter_components
...
from pi4home.py_compat import IS_PY3, string_types
from pi4home.storage_json import StorageJSON, storage_path
...
CPP_BASE_FORMAT = (u"""// Auto generated code by pi4home
""", u""""
...
INI_BASE_FORMAT = (u"""; Auto generated code by pi4home
...
def get_include_text():
    include_text = u'#include "pi4home.h"\n' \
                   u'using namespace pi4home;\n'
    for _, component, conf in iter_components(CORE.config):
...
def update_pi4home_core_repo():
    ...
    if rc != 0:
        ...
        return
    rc, _, _ = run_system_command('git', '-C', pi4home_core_path, 'diff-index', '--quiet', 'HEAD',
                                  '--')
    if rc != 0:
        # local changes, cannot update
        _LOGGER.warning("Local changes in pi4home-core copy from git. Will not auto-update.")
        return
    _LOGGER.info("Updating pi4home-core copy from git (%s)", pi4home_core_path)
    rc, stdout, _ = run_system_command('git', '-c', 'color.ui=always', '-C', pi4home_core_path,
                                       'pull', '--stat')
    if rc != 0:
...
def migrate_src_version_0_to_1():
    ...
    content, count = replace_file_content(content, r'using namespace pi4homelib;', '')
    if count != 0:
        _LOGGER.info("Migration: Removed %s occurrence of 'using namespace pi4homelib;' "
                     "in %s", count, main_cpp)

    if CPP_INCLUDE_BEGIN not in content:
        content, count = replace_file_content(content, r'#include "pi4homelib/application.h"',
                                              CPP_INCLUDE_BEGIN + u'\n' + CPP_INCLUDE_END)
        if count == 0:
            _LOGGER.error("Migration failed. PI4Home 1.10.0 needs to have a new auto-generated "
...
def storage_should_clean(old, new):  # type: (StorageJSON, StorageJSON) -> bool
    ...
    if old.pi4home_core_version != new.pi4home_core_version:
        return True
    if old.pi4home_version != new.pi4home_version:
        return True
...
def update_storage_json():
    ...
    new = StorageJSON.from_pi4home_core(CORE, old)
    if old == new:
        return
...
def gather_lib_deps():
    lib_deps = set()
    if CONF_REPOSITORY in CORE.pi4home_core_version:
        repo = CORE.pi4home_core_version[CONF_REPOSITORY]
        ref = next((CORE.pi4home_core_version[x] for x in (CONF_COMMIT, CONF_BRANCH, CONF_TAG)
                    if x in CORE.pi4home_core_version), None)
        if CONF_TAG in CORE.pi4home_core_version and repo == LIBRARY_URI_REPO:
            this_version = GITHUB_ARCHIVE_ZIP.format(ref)
        elif ref is not None:
            this_version = repo + '#' + ref
        lib_deps.add(this_version)
    elif CORE.is_local_pi4home_core_copy:
        src_path = CORE.relative_path(CORE.pi4home_core_version[CONF_LOCAL])
        ...
        lib_deps.add(CORE.pi4home_core_version)
...
def write_platformio_ini(content, path):
    symlink_pi4home_core_version(CORE.pi4home_core_version)
    update_pi4home_core_repo()
    update_storage_json()
...
GITIGNORE_CONTENT = """# Gitignore settings for PI4Home
