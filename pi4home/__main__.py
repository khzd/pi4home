...
from pi4home import const, core_config, mqtt, platformio_api, wizard, writer, yaml_util
from pi4home.api.client import run_logs
...
from pi4home.util import run_external_command, run_external_process, safe_print, \
    is_dev_pi4home_version

_LOGGER = logging.getLogger(__name__)

PRE_INITIALIZE = ['pi4home', 'logger', 'wifi', 'ethernet', 'ota', 'mqtt', 'web_server', 'api',
                  'i2c']
...
def compile_program(args, config):
    ...
    if rc != 0 and CORE.is_dev_pi4home_core_version and not is_dev_pi4home_version():
        _LOGGER.warning("You're using 'pi4home_core_version: dev' but not using the "
                        "dev version of the PI4Home tool.")
        _LOGGER.warning("Expect compile errors if these versions are out of sync.")
        _LOGGER.warning("Please install the dev version of PI4Home too when using "
                        "'pi4home_core_version: dev'.")
        _LOGGER.warning(" - Hass.io: Install 'PI4Home (dev)' addon")
        _LOGGER.warning(" - Docker: docker run [...] pi4home/pi4home:dev [...]")
        _LOGGER.warning(" - PIP: pip install -U https://github.com/khzd/pi4home/pi4home/archive/dev.zip")
    return rc
...
def upload_program(config, args, host):
    ...
    from pi4home.components import ota
    from pi4home import espota2

    if args.host_port is not None:
        host_port = args.host_port
    else:
        host_port = int(os.getenv('PI4HOME_OTA_HOST_PORT', random.randint(10000, 60000)))
...
def command_dashboard(args):
    from pi4home.dashboard import dashboard
...
def parse_args(argv):
    parser = argparse.ArgumentParser(prog='pi4home')
    parser.add_argument('-v', '--verbose', help="Enable verbose pi4home logs.",
                        action='store_true')
    ...
    subparsers.add_parser('wizard', help="A helpful setup wizard that will guide "
                                         "you through setting up pi4home.")

    subparsers.add_parser('mqtt-fingerprint', help="Get the SSL fingerprint from a MQTT broker.")

    subparsers.add_parser('version', help="Print the pi4home version and exit.")
    ...
    dashboard.add_argument("--hassio",
                           help="Internal flag used to tell pi4home is started as a Hass.io "
                                "add-on.",
                           action="store_true")
