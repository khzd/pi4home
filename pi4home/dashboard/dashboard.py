...
from pi4home.zeroconf import DashboardStatus, Zeroconf
...
RELATIVE_URL = os.getenv('PI4HOME_DASHBOARD_RELATIVE_URL', '/')
STATUS_USE_PING = get_bool_env('PI4HOME_DASHBOARD_USE_PING')
...
def template_args():
    ...
    return {
        'version': version,
        'docs_link': 'https://beta.github.com/khzd/pi4home/' if 'b' in version else 'https://github.com/khzd/pi4home/',
        'get_static_file_url': get_static_file_url,
        'relative_url': RELATIVE_URL,
        'streamer_mode': get_bool_env('PI4HOME_STREAMER_MODE'),
    }
...
class pi4homeHassConfigHandler(pi4homeCommandWebSocket):
    def build_command(self, message):
        js = json.loads(message)
        config_file = os.path.join(CONFIG_DIR, js['configuration'])
        return ["pi4home", "--dashboard", config_file, "hass-config"]
...
class DashboardEntry(object):
    ...
    def update_old(self):
        if self.storage is None:
            return ''
        return self.storage.pi4home_version or ''
...
def start_web_server(args):
    ...
    mkdir_p(CONFIG_DIR)
    mkdir_p(os.path.join(CONFIG_DIR, ".pi4home"))
