...
from pi4home.const import CONF_ARDUINO_VERSION, CONF_PI4HOME, CONF_PI4HOME_CORE_VERSION, \
    CONF_LOCAL, CONF_USE_ADDRESS, CONF_WIFI, ESP_PLATFORM_ESP32, ESP_PLATFORM_ESP8266, \
    CONF_REPOSITORY, CONF_BRANCH
from pi4home.helpers import ensure_unique_string, is_hassio
from pi4home.py_compat import IS_PY2, integer_types
...
class MACAddress(object):
    ...
    def as_hex(self):
        from pi4home.cpp_generator import RawExpression
...
class ID(object):
    ...
    def resolve(self, registered_ids):
        from pi4home.config_validation import RESERVED_IDS
...
class pi4homeCore(object):
    ...
    @property
    def pi4home_core_version(self):  # type: () -> Dict[str, str]
        return self.config[CONF_PI4HOME][CONF_PI4HOME_CORE_VERSION]

    @property
    def is_dev_pi4home_core_version(self):
        if CONF_REPOSITORY not in self.pi4home_core_version:
            return False
        return self.pi4home_core_version.get(CONF_BRANCH) == 'dev'

    @property
    def is_local_pi4home_core_copy(self):
        return CONF_LOCAL in self.pi4home_core_version

    @property
    def arduino_version(self):  # type: () -> str
        return self.config[CONF_PI4HOME][CONF_ARDUINO_VERSION]
    ...
    def add(self, expression, require=True):
        from pi4home.cpp_generator import Expression
