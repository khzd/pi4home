...
.. seo::
    :description: Instructions for setting up the MQTT client to communicate with the local network in PI4Home.
    :image: mqtt.png
    :keywords: MQTT

The MQTT Client Component sets up the MQTT connection to your broker and
is currently required for PI4Home to work. In most cases, you will
...
.. warning::
    ...
    remove the ``api:`` line from your PI4Home configuration, otherwise the ESP will
...
With the MQTT Message schema you can tell PI4Home how a specific MQTT message should be sent.
...
Using PI4Home with Home Assistant is easy, simply setup an MQTT
broker (like `mosquitto <https://mosquitto.org/>`__) and point both your
Home Assistant installation and PI4Home to that broker. Next, enable
...
And that should already be it 🎉 All devices defined through PI4Home should show up automatically
...
To fix this, PI4Home has a simple helper script that purges stale
retained messages for you:

.. code-block:: bash

    pi4home configuration.yaml clean-mqtt
...
By default, PI4Home will prefix all messages with your node name or
...
``home/+/#`` together with PI4Home. All other features of PI4Home
...
By default, PI4Home will send a retained MQTT message to
...
then run the ``mqtt-fingerprint`` script of PI4Home to get the certificate:

.. code-block:: bash

    pi4home livingroom.yaml mqtt-fingerprint
...
All components in PI4Home that do some sort of communication through
