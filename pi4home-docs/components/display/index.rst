...
The ``display`` component houses PI4Home's powerful rendering and display
...
For the last type, PI4Home has a powerful rendering engine that can do
many things like draw some basic shapes, print text with any font you want, or even show images.

To achieve all this flexibility displays tie in directly into PI4Home's :ref:`lambda system <config-lambda>`.
...
In this section we will be discussing how to use PI4Home's display rendering engine from PI4Home
and some basic commands. Please note that this only applies to displays that can control each pixel
individually.

So, first a few basics: When setting up a display platform in PI4Home there will be a configuration
option called ``lambda:`` which will be called every time PI4Home wants to re-render the display.
In there, you can write code like in any :ref:`lambda <config-lambda>` in PI4Home. Display
...
Now that you know a bit more about PI4Home's coordinate system, let's draw some basic shapes like lines, rectangles
and circles:
...
PI4Home only has implemented binary displays.
...
The rendering engine also has a powerful font drawer which integrates seamlessly into PI4Home.
Whereas in most arduino display projects you have to use one of a few pre-defined fonts in very
specific sizes, with PI4Home you have the option to use **any** truetype (``.ttf``) font file
at **any** size! Granted the reason for it is actually not having to worry about the licensing of font files :)

To use fonts you first have to define a font object in your PI4Home configuration file. Just grab
...
.. note::
    ...
    To use fonts you will need to have the python ``pillow`` package installed, as PI4Home uses that package
    to translate the truetype files into an internal format. If you're running this as a Hass.io add-on or with
    the official PI4Home docker image, it should already be installed. Otherwise you need to install it using
    ``pip2 install pillow``.
...
By default, PI4Home will *align* the text at the top left. That means if you enter the coordinates
...
When you enter ``[0,10]`` you're really telling PI4Home that it should position the **anchor point** of the text
...
on the display!. That's where ``printf`` comes in. ``printf`` is a formatting engine from the C era and PI4Home
...
but for PI4Home there are really just a few things you need to know.
...
With PI4Home you can also display the current time using the NTP protocol. Please see the example :ref:`here <strftime>`.
...
.. note::
    ...
    If you're running this as a Hass.io add-on or with the official PI4Home docker image, it should already be
