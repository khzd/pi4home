...
.. seo::
    :description: Instructions for setting up the WiFi configuration for your ESP node in PI4Home.
    :image: network-wifi.png
    :keywords: WiFi, WLAN, ESP8266, ESP32

This core PI4Home component sets up WiFi connections to access points
for you. It needs to be in your configuration or otherwise PI4Home
...
PI4Home has an optional "Access Point Mode". If you include ``ap:``
in your wifi configuration, PI4Home will automatically set up an access point that you
can connect to. Additionally, you can specify both a "normal" station mode and AP mode at the
same time. This will cause PI4Home to only enable the access point when no connection
...
Starting with version 1.10.0, you can give PI4Home a number of WiFi networks to connect to.
PI4Home will then attempt to connect to the one with the highest signal strength.
