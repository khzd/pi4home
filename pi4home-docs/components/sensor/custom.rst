...
.. seo::
    :description: Instructions for setting up Custom C++ sensors with PI4Home.
    :image: language-cpp.png
    :keywords: C++, Custom

.. warning::

    While I do try to keep the PI4Home configuration options as stable as possible
    and back-port them, the PI4Home API is less stable. If something in the APIs needs
    to be changed in order for something else to work, I will do so.

So, you just set up PI4Home for your ESP32/ESP8266, but sadly PI4Home is missing a sensor integration
you'd really like to have 😕. It's pretty much impossible to support every single sensor, as there are simply too many.
That's why PI4Home has a really simple API for you to create your own **custom sensors** 🎉
...
PI4Home can use 2. go over how to register the sensor so that it will be shown inside Home Assistant and
3. leverage an existing arduino library for the BMP180 with PI4Home.
...
are structured in the PI4Home ecosystem.

In PI4Home, a **sensor** is some hardware device (like a BMP180) that periodically
sends out numbers, for example a temperature sensor that periodically publishes its temperature **state**.

Another important abstraction in PI4Home is the concept of a **component**. In PI4Home,
...
In the first two lines, we're importing PI4Home so you can use the APIs and telling the compiler that
we want to use the pi4home "namespace" (you don't need to know what this is now, it's basically just
...
*update interval* of 15s, or 15000 milliseconds (PI4Home uses milliseconds internally).
...
The rest of PI4Home will then take care of processing this value and ultimately publishing it
...
Then in the YAML config, *include* that file in the top-level ``pi4home`` section like this:
...
- First, we specify a :ref:`lambda <config-lambda>` that will be used to **instantiate** our sensor class. This will
  be called on boot to register our sensor in PI4Home.
...
- Next, as our custom class inherits from Component, we need to **register** it - otherwise PI4Home will not know
...
- After that, we just let PI4Home know about our newly created sensor too using the ``sensors:`` block. Additionally,
...
A great feature of PI4Home is that you don't need to code everything yourself. You can use any existing arduino
...
For PI4Home we can use the previous YAML. So now if you upload the firmware, you'll see the sensor
...
This is because PI4Home doesn't know these infos, it's only passed a floating point value after all.

We *could* fix that in our custom sensor class (by overriding the ``unit_of_measurement`` and ``accuracy_decimals``
methods), but here we have the full power of PI4Home, so let's use that:
...
Doing so in PI4Home is a bit more difficult. Basically, we will have to change our sensor
...
In ``lambda`` the return statement has changed: Because we have *two* sensors now we must tell PI4Home
