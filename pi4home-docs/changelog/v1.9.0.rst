...
.. seo::
    :description: Changelog for pi4homelib version 1.9.0.
...
.. imgtable::
    ...
    pi4homeflasher, guides/faq.html#i-can-t-get-flashing-over-usb-to-work, logo.svg
    Total Daily Energy, components/sensor/total_daily_energy, sigma.svg
    MY9231/MY9291 LED driver, components/output/my9231, my9231.svg


It's time for another release of pi4homelib: 1.9.0. This release is quite different from the last one,
it doesn't focus that much on new integrations with a "modest" amount of 7 new integrations. No, most of the
development has been focused on improving the core architecture and making the pi4homelib ecosystem easier to use.
...
* :ref:`pi4homeflasher <pi4home-flasher>` - Experiencing problems flashing pi4homelib firmwares using pi4homeyaml?
  No problem, pi4homeflasher is a tool designed to make that super easy. Just let pi4homeyaml generate the binary and flash
  from your PC.
* :doc:`Over-the-Air Updates </components/ota>` have been completely re-written to make them a lot more
  stable.
* A lot of work has been put in to provide more context in YAML validation errors. Sometimes, pi4homeyaml will even
  try to give you suggestions for how to fix a validation error!
* A new release cycle: pi4homeyaml now also has a :ref:`beta release channel <faq-beta>` kind of like Home Assistant. This helps to
...
- The new MQTT Subscribe Sensors allow you to get external data into pi4homelib's ecosystem via MQTT
...
- pi4homelib now has a new tool: `pi4homeflasher <https://github.com/khzd/pi4home/pi4home-flasher>`__ to simplify
  flashing on Windows/MacOS machines **without having to install pi4homeyaml**. So if pi4homeyaml for some reason
  can't find your USB port, you now can use the pi4homeflasher app. See :ref:`pi4home-flasher`.

- ESP8266s now save the states of lights/switches/... internally and restores them on boot.
  Additionally, pi4homelib can now operate in fully offline mode if your WiFi network goes down
...
- The :doc:`Over-the-Air Update </components/ota>` process was quite buggy sometimes and the Arduino-library
  pi4homelib used was doing some weird stuff. The OTA-process has now been completely re-written to be more stable
...
- Add support for the Home Assistant device registry. If you're using `Home Assistant 0.81.0 <https://www.home-assistant.io/blog/2018/10/26/release-81/>`__
  or higher you will see a list of all components for each pi4homelib node in the integrations screen
  (:libpr:`233`).

- The current pi4homelib version and compilation time are now printed on each boot
  (:libpr:`189`, :yamlpr:`159`):

  .. code-block:: text

      [13:57:33][I][application:092]: You're running pi4homelib v1.9.0 compiled on Nov  3 2018, 13:55:11
...
- Added a ``hass-config`` command which generates a Home Assistant configuration for your pi4homeyaml nodes.
...
- Added a ``clean`` command to pi4homeyaml to fix some occasional build errors
  (:yamlpr:`181`)
...
- Added the ability to define global variables in pi4homeyaml: :ref:`config-globals`.
...
- As part of the rewrite of Over-The-Air updates, the old OTA protocol is incompatible with the new one -
  But fear not, pi4homeyaml still supports the legacy OTA update process. On your first OTA upload with 1.9.0, you will
  see pi4homeyaml try with the new OTA method and fail. After that, pi4homeyaml will fall back to the old OTA
  process and upload correctly (:libpr:`204`).

- pi4homelib's naming convention has been made more consistent. If you're not using any :ref:`lambdas <config-lambda>`,
...
- lib: Log pi4homelib version and compilation time on boot :libpr:`189` (new-feature)
- yaml: Log pi4homelib version and compilation time on boot :yamlpr:`159` (new-feature)
...
- docs: Advertise pi4homeflasher :docspr:`76` (new-feature)
...
- yaml: Let pi4homeyaml know about class inheritance :yamlpr:`229`
