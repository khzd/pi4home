...
.. seo::
    :description: Getting started guide for automations in PI4Home.
    :image: auto-fix.png

Automations and templates are two very powerful concepts of PI4Home. Automations
allow you to perform actions under certain conditions and templates are a way to easily
customize everything about your node without having to dive into the full PI4Home C++
API.
...
That's why, starting with PI4Home 1.7.0, there's a new automation engine. With it, you
...
Now comes the actual automation block. With ``then``, you tell PI4Home what should happen when the press happens.
...
Now that concludes the introduction into automations in PI4Home. They're a powerful tool to automate almost
everything on your device with an easy-to-use syntax. For the cases where the "pure" YAML automations don't work,
PI4Home has another extremely powerful tool to offer: Templates.
...
With templates inside PI4Home, you can do almost *everything*. If for example you want to only perform a certain
...
tells PI4Home that the following block is supposed to be interpreted as a lambda, or C++ code. Note that
...
Finally, ``id(...)`` is a helper function that makes PI4Home fetch an object with the supplied ID (which you defined
somewhere else, like ``top_end_stop```) and let's you call any of PI4Home's many APIs directly. For example, here
we're retrieving the current state of the end stop using ``.state`` and using it to construct our cover state.

.. note::

    PI4Home does not check the validity of lambda expressions you enter and will blindly copy
...
Another feature of PI4Home is that you can template almost every parameter for actions in automations. For example
...
YES! All automations you define in PI4Home are execute on the ESP itself and will continue to
work even if the WiFi network is down or the MQTT server is not reachable.

There is one caveat though: PI4Home automatically reboots if no connection to the MQTT broker can be
...
- :ref:`pi4home.on_boot <pi4home-on_boot>` / :ref:`pi4home.on_shutdown <pi4home-on_shutdown>` / :ref:`pi4home.on_loop <pi4home-on_loop>`
