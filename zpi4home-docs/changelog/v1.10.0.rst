...
.. seo::
    :description: Changelog for pi4homelib version 1.10.0.
...
So I decided to do something against that: PI4Home now features a native protocol (based on TCP+protocol buffers)
that addresses these issues and has a
`component on the Home Assistant <https://rc--home-assistant-docs.netlify.com/components/pi4home/>`__
side too.
...
after some quick changes, PI4Home is now also compatible with python 3.5+. So with this new release
you can try PI4Home with Python 3 already. Just make sure to install the development version of platformio first:
...
PI4Home is to drop python 2 as soon as possible. Once platformio releases version 4.0 (with python 3 support), I
will go through and check everything still works. Maybe PI4Home will support python 2 for 1 or 2 releases after that
...
Rename: pi4home{lib, yaml} -> PI4Home
-------------------------------------

The name pi4homelib and pi4homeyaml were too technical, and this project has changed a lot since the first
published release (the yaml part didn't even exist back then). So ... the project is being re-branded slightly:

- pi4homeyaml -> PI4Home
- pi4homelib -> PI4Home Core
- pi4homedocs -> PI4Home Docs
...
have long been inaccurate now and users should really switch over to PI4Home through YAML
...
This migration is of course huge - almost every single file in the code+docs base has pi4home{lib, yaml} somewhere
in it, so it will take time until the next release for this to be finished.

Breaking Changes
----------------

- Previously, pi4homelib would by default only publish every 15th sensor value in order to provide averaged values.
...
- Added a bunch of guides (and helpers) for creating custom components. Also new: ``pi4homeyaml.libraries``,
  ``pi4homeyaml.includes`` and ``pi4homeyaml.platformio_options`` (:doc:`/components/pi4home`)
- Saved a lot of flash space on ESP8266 boards. Previously, platformio would allocate about 1/4 of flash for SPIFFS,
  but pi4homelib doesn't use that so now you have that as extra storage. Thanks `@brandond <http://github.com/brandond>`
- You can now use Home Assistant to get time in PI4Home, so no more need for SNTP. See :ref:`time`.
...
- lib: Native pi4homelib API :libpr:`322`
- yaml: Add native PI4Home API :yamlpr:`265`
