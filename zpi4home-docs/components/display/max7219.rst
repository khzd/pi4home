...
with PI4Home. Please note that this integration is *only* for 7-segment display, not matrix configurations.
...
at the top of this page. But even though you're passing in a string (here ``"01234567"``), PI4Home converts it
...
Also note that the ``.`` (dot) character is special because when PI4Home encounters it in the string the dot
